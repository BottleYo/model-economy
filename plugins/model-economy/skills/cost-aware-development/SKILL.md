...
- `core`（核心模式）：目录安装后四个 Skill 可直接运行，但没有本地六角色配置。简单任务由主 agent 执行；机械和标准任务只有在用户明确授权创建且当前工具可用时才可使用通用可见任务执行者，继续应用原生质量门，不尝试调用不存在的自定义角色。
...
[路由策略](references/routing-policy.json)是任务分类、角色集合、执行目标、条件门、完成门和预算的唯一机器事实源。按 `classification_order` 和 `first_match` 选择唯一类别；最后的 `standard` 是默认 fallback，不因文件数、请求篇幅或省 token 要求而降级风险。
...
1. 大型/高风险：只读 `model-economy-architect`（`strong`）必须在设计审批前输出架构边界、风险和决策，且不得写计划、补丁或代码。审批后按模式执行矩阵恰好选择一个主执行目标；获授权且所有可见条件满足时可以选择 `visible_task`，但核心模式仍保留角色隔离关口。验证后必须由 `model-economy-final-reviewer`（`strong`）复核才能完成。
2. 机械：只有满足全部 economy 条件时才允许 `model-economy-batch-worker`。简单：由主 agent 用 `balanced` 直接完成，不启动 subagent。
3. 标准：按模式执行矩阵恰好选择主 agent、`model-economy-implementer` 或获授权的 `visible_task` 承担主要实现。文件位置、依赖或事实不明确时才允许 explorer；跨模块、关键逻辑、非显然回归风险或实质测试疑问出现时才允许 reviewer。两次实质失败，或有证据的具体能力不匹配时，才允许有界的只读 architect 诊断；发现高风险时重新分类。

## 可见任务与工作包

`visible_task` 是通用主执行目标，不是第七个自定义角色、implementer/batch-worker 的身份别名，也不具备六角色的同等沙箱。每个实施单元恰好选择一个主执行目标；选择可见任务后，主 agent 不重做同一实现，也不得为同一范围启动内部 implementer/batch-worker。

只有当前用户明确要求创建任务，或可信项目规则已授权按工作包创建独立任务，才可使用可见任务。泛泛的“开始实施”不构成新建授权；当前宿主工具要求优先。工具、项目、起点、模型组合或权限不满足时停止并报告，不能静默换模型、盲目重建或以内部 subagent 替代严格可见范围。

默认 `auto`：小任务留在当前会话；`visible-first` 仅优先已获授权的可见委派；`current-only` 不主动派发。用户要求所有委派可见时，默认覆盖实施、探索、架构和审查；若宿主不能给可见审查提供可核对只读权限，大型/高风险完整路线不可用，必须报告缺口或取得明确降级/范围变更批准。

工作包、复用、创建中状态、模型请求、预算、继续任务和集成步骤按需读取[可见任务契约](references/visible-tasks.md)。低上下文或不涉及派发的任务不读取该参考。继续任务时省略模型/推理字段才表示沿用既有请求；新建任务省略时使用宿主或用户默认。两者都不能称为降档；工具未回显运行配置时只记录请求，不能推断身份或实际模型。
