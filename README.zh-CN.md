...
> **v0.7.0：精细路由与可见任务契约。** 经维护者批准跳过真实桌面派发试点，以正式版发布。自动化测试已通过；真实任务创建、续改及复杂并行尚未验证。升级注意事项和限制见[中英文发行说明](docs/release/0.7.0.md)。
...
[v0.6.1 稳定版流程图（旧预算，仅供该版本参考）](assets/model-economy-flow-zh-CN.svg)
